...
def compile_shaders():
    vertex_shader_source = """
        ...
        layout (location = 1) in vec4 pick_color;
        ...
        void main(void) {
            ...
            gl_Position = P_matrix * V_matrix * M_matrix *
              (position + (gl_InstanceID % 10) * vec4(1.0, 0.0, 0.0, 0.0) + (next_line * vec4(0.0, 1.0, 0.0, 0.0)));

            vertex_color = pick_color;
        }
    """
...
def render(time):
    ...
    M_matrix = glm.rotate(glm.mat4(1.0), time, glm.vec3(1.0, 1.0, 0.0)) #rotation settings
    ...
    #move square of squares to mid
    M_matrix = glm.translate(M_matrix, glm.vec3(-4.0, -4.0, 0.0))
    glUniformMatrix4fv(M_location, 1, GL_FALSE, glm.value_ptr(M_matrix))

    #draw_method_1(M_location, M_matrix)

    glDrawArraysInstanced(GL_TRIANGLES, 0, 36, 100)

def draw_method_1(M_location, M_matrix):
    for j in range(10):
...
def main():
    ...
    window = glfwCreateWindow(400, 400, "Lab7", None, None)
    if not window:
        glfwTerminate()
        sys.exit(-1)
